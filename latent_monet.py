...
PATH = '/home/diegushko/dataset/monet2photo'
...
class ContentEncoder(nn.Module):
    ...
    def __init__(self, base_channels=64, n_downsample=2, n_res_blocks=4):
        ...
        # Residual blocks
        layers += [
            ResidualBlock(channels) for _ in range(n_res_blocks) #number of iterations 16
        ]
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, n_upsample=2, n_res_blocks=4, s_dim=8, h_dim=256):
        ...
        self.res_blocks = nn.ModuleList([
            ResidualBlock(channels, s_dim) for _ in range(n_res_blocks) #number of iterations 16
        ])
        ...
        for i in range(n_upsample):
            '''
            the shape of the normalized layer is torch.Size([1, 128, 128, 128])
            the shape of the normalized layer is torch.Size([1, 64, 256, 256])
            the shape of the normalized layer is torch.Size([1, 128, 128, 128])
            the shape of the normalized layer is torch.Size([1, 64, 256, 256])
            the shape of the normalized layer is torch.Size([1, 128, 128, 128])
            the shape of the normalized layer is torch.Size([1, 64, 256, 256])
...
class Generator(nn.Module):
    ...
    def __init__(
        self,
        base_channels: int = 64, #<======== changed from 64 to 32
...
munit_test = MUNIT_test(**munit_config).to(device)

# Initialize optimizers
gen_params = list(munit_test.gen_a.parameters()) + list(munit_test.gen_b.parameters())
dis_params = list(munit_test.dis_a.parameters()) + list(munit_test.dis_b.parameters())
...
if pretrained:
    ...
    munit_test.gen_a.load_state_dict(pre_dict['gen_a'])
    munit_test.gen_b.load_state_dict(pre_dict['gen_b'])
    gen_optimizer.load_state_dict(pre_dict['gen_opt'])
    munit_test.dis_a.load_state_dict(pre_dict['disc_a'])
    munit_test.dis_b.load_state_dict(pre_dict['disc_b'])
    dis_optimizer.load_state_dict(pre_dict['dis_opt'])
else:
    gen_AB = munit_test.gen_a.to(device).apply(weights_init)
    gen_BA = munit_test.gen_b.to(device).apply(weights_init)
    disc_A = munit_test.dis_a.to(device).apply(weights_init)
    disc_B = munit_test.dis_b.to(device).apply(weights_init)
...
with torch.no_grad():
    for i in range(9):
        cuda_img = iter(test_loader).next().to(device)
        s_a = torch.randn((1, 8, 1, 1), device=device)
        s_b = torch.randn((1, 8, 1, 1), device=device)
        x_ba = munit_test(cuda_img, cuda_img, s_a, s_b)
        #real_b = nn.functional.interpolate(cuda_img, size=target_shape)
        #catenation = torch.cat([real_b, x_ba])
        #image_tensor = (catenation + 1) / 2
        image_shifted = (x_ba + 1) / 2
        image_unflat = image_shifted.detach().cpu().view(-1, *(dim_A, target_shape, target_shape))

        image_grid = make_grid(image_unflat[:25], nrow=5)

        axs[i].imshow(image_grid.permute(1, 2, 0).squeeze())
        axs[i].set_title(f'iter {i}')

plt.savefig(output_ + 'figure_latent_monet.png')
#plt.show()
