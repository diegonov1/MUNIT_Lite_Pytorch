...
checkpoint_directory = '/content/drive/MyDrive/checkpoint/munit/art2photo'
...
'recon_x_w': 8, #10,                 # weight of image reconstruction loss
'recon_s_w': 2, #1,                  # weight of style reconstruction loss
'recon_c_w': 2, #1,                  # weight of content reconstruction loss
...
'mlp_dim': 512, #256,                    # number of filters in MLP
...
'num_workers': 2, #8,                              # number of data loading threads
'new_size': 512, #256,                               # first resize the shortest image side to this size
'crop_image_height': 512, #256,                      # random crop image of this height
'crop_image_width': 512, #256,                       # random crop image of this width
'data_root': '/content/drive/MyDrive/datasets/image2image/art2photo'   # dataset folder location
}
...
class MUNIT_Trainer(nn.Module):
    ...
    def resume(self, checkpoint_dir, params):
        ...
        # Load optimizers
        state_dict = torch.load(os.path.join(checkpoint_dir, 'optimizer_SD.pt'), map_location='cuda:0')
    ...
    def save(self, snapshot_dir, iterations):
        ...
        gen_name = os.path.join(snapshot_dir, 'gen_SD_%08d.pt' % (iterations + 1))
        dis_name = os.path.join(snapshot_dir, 'dis_SD_%08d.pt' % (iterations + 1))
        opt_name = os.path.join(snapshot_dir, 'optimizer_SD.pt')
...
while True:
    for it, (images_a, images_b) in enumerate(zip(train_loader_a, train_loader_b)):
        ...
        if (iterations + 1) % params['image_save_iter'] == 0:
            ...
            write_2images(test_image_outputs, display_size, image_directory, 'test_SD_%08d' % (iterations + 1))
            write_2images(train_image_outputs, display_size, image_directory, 'train_SD_%08d' % (iterations + 1))
            # HTML
        ...
        if (iterations + 1) % params['snapshot_save_iter'] == 0:
            ...
            if itera > 0:
                os.remove(checkpoint_directory + '/gen_SD_%08d.pt' % (itera))
                os.remove(checkpoint_directory + '/dis_SD_%08d.pt' % (itera))
