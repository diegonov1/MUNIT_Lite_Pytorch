...
checkpoint_directory = '/home/diegushko/checkpoint/vangogh2photo'
...
'mlp_dim': 384, #256,                    # number of filters in MLP
...
'new_size': 384, #256,                               # first resize the shortest image side to this size
'crop_image_height': 384, #256,                      # random crop image of this height
'crop_image_width': 384, #256,                       # random crop image of this width
...
class MUNIT_Trainer(nn.Module):
    ...
    def save(self, snapshot_dir, iterations):
        ...
        gen_name = os.path.join(snapshot_dir, 'gen_vg_%08d.pt' % (iterations + 1))
        dis_name = os.path.join(snapshot_dir, 'dis_vg_%08d.pt' % (iterations + 1))
        opt_name = os.path.join(snapshot_dir, 'optimizer_vg.pt')
...
while True:
    for it, (images_a, images_b) in enumerate(zip(train_loader_a, train_loader_b)):
        ...
        if (iterations + 1) % params['image_save_iter'] == 0:
            ...
            write_2images(test_image_outputs, display_size, image_directory, 'test_vg_%08d' % (iterations + 1))
            write_2images(train_image_outputs, display_size, image_directory, 'train_vg_%08d' % (iterations + 1))
            # HTML
